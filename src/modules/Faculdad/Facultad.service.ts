...
export class FacultadService{
  ...
  public async getAll(): Promise<any> {
    ...
    const query = `MATCH (n:Facultad) return n`;
    return session
        .run(query)
        .then((result) => {
        session.close();
  ...
  public async getConsultarPrograma(facu): Promise<any> {
    const session = this.neo4j.session();
    const query = `Match (P:Programa)-[:HacePartede]->(F:Facultad{NombreFacultad:"`+facu+`"}) return P`;
    console.log(query)
    return session
        .run(query)
        .then((result) => {
